...
app.put("/api/users/:student_id", (req, res) => {
    ...
    const result = updateUser.run(balance, JSON.stringify(possession_list), Number(student_id));
    if (result.changes === 0) {
...
app.post("/api/add-product", (req, res) => {
    ...
    const updateUser = db.prepare("UPDATE users SET balance = ?, possession_list = ? WHERE student_id = ?");
    const result = updateUser.run(newBalance, JSON.stringify(possessionList), Number(student_id));
    if (result.changes === 0) {
        console.log("User not found or not updated:", student_id);
        return res.status(404).json({ error: "User not found or not updated" });
    }
